...
ssh -o StrictHostKeyChecking=no -i $SSH_KEY ubuntu@*.*.*.*

echo "copying flask app"
scp -r -i $SSH_KEY ./flask-app ubuntu@*.*.*.*:/home/ubuntu
...
ssh -o StrictHostKeyChecking=no -i $SSH_KEY ubuntu@*.*.*.* 'bash -s' < ./bash-script-automation.sh
